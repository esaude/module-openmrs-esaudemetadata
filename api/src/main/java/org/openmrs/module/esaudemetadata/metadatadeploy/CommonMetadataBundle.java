...
public class CommonMetadataBundle extends AbstractMetadataBundle {
    ...
    public void install() throws Exception {
        ...
        install(encounterType("MISAU: ATS", "Aconselhamento e Testagem de Saude", MetadataBundleUtils._EncounterType.MISAU_ATS));
        install(encounterType("MISAU: ITS", "REGISTO DIARIO DE CASOS DE DTS", MetadataBundleUtils._EncounterType.MISAU_ITS));
        install(encounterType("MISAU: LABORATORIO", "Laboratorio", MetadataBundleUtils._EncounterType.MISAU_LABORATORIO));
        install(encounterType("POC: CURRENT STORY", "A consultation where the clinician writes the notes about the current clinical observations of the patient", MetadataBundleUtils._EncounterType.POC_CURRENT_STORY));
        install(encounterType("POC:DISPENSATION", "For drugs dispensation on the pharmacy module - poc", MetadataBundleUtils._EncounterType.POC_DISPENSATION));
        install(encounterType("PTV: MATERNIDADE", "Prevencao de transmissao vertical na maternidade", MetadataBundleUtils._EncounterType.PTV_MATERNIDADE));
        install(encounterType("PTV: PRE-NATAL INICIAL", "Prevenção Transmisiveis Vertical", MetadataBundleUtils._EncounterType.PTV_PRE_NATAL_INICIAL));
        install(encounterType("PTV: PRE-NATAL SEGUIMENTO", "Prevenção Transmisiveis Vertical", MetadataBundleUtils._EncounterType.PTV_PRE_NATAL_SEGUIMENTO));
        install(encounterType("S.TARV: ACONSELHAMENTO", "MISAU/HDD TARV Aconselhamento", MetadataBundleUtils._EncounterType.S_TARV_ACONSELHAMENTO));
        install(encounterType("S.TARV: ACONSELHAMENTO SEGUIMENTO", "Seguimento Aconselhamento", MetadataBundleUtils._EncounterType.S_TARV_ACONSELHAMENTO_SEGUIMENTO));
        install(encounterType("S.TARV: ADULTO INICIAL B", "Outpatient Adult Initial Visit part 2", MetadataBundleUtils._EncounterType.S_TARV_ADULTO_INICIAL_B));
        install(encounterType("S.TARV: ADULTO SEGUIMENTO", "seguimento visita do paciente adulto", MetadataBundleUtils._EncounterType.S_TARV_ADULTO_SEGUIMENTO));
        install(encounterType("S.TARV: AVALIACAO DE ADESAO", "Avaliacao de adesao para pacientes em TARV", MetadataBundleUtils._EncounterType.S_TARV_AVALIACAO_DE_ADESAO));
        install(encounterType("S.TARV: AVALIACAO E PREPARACAO DO CANDIDATO TARV", "Avalicao e preparacao psicologica do candidato ao TARV", MetadataBundleUtils._EncounterType.S_TARV_AVALIACAO_E_PREPARACAO_DO_CANDIDATO_TARV));
        install(encounterType("S.TARV: BUSCA ACTIVA", "Busca Activa", MetadataBundleUtils._EncounterType.S_TARV_BUSCA_ACTIVA));
        install(encounterType("S.TARV: FARMACIA", "Farmacia", MetadataBundleUtils._EncounterType.S_TARV_FARMACIA));
        install(encounterType("S.TARV: LIVRO PRE-TARV", "Representa o livro de registo PRE-TARV", MetadataBundleUtils._EncounterType.S_TARV_LIVRO_PRE_TARV));
        install(encounterType("S.TARV: LIVRO TARV", "Representa o livro de registo TARV", MetadataBundleUtils._EncounterType.S_TARV_LIVRO_TARV));
        install(encounterType("S.TARV: PEDIATRIA INICIAL A", "Representa o livro de registo TARV", MetadataBundleUtils._EncounterType.S_TARV_PEDIATRIA_INICIAL_A));
        install(encounterType("S.TARV: PEDIATRIA INICIAL B", "Primeira visita de paciente pediatrico, parte B. As duas ultimas paginas", MetadataBundleUtils._EncounterType.S_TARV_PEDIATRIA_INICIAL_B));
        install(encounterType("S.TARV: PEDIATRIA SEGUIMENTO", "Seguimento visita do paciente pediatria", MetadataBundleUtils._EncounterType.S_TARV_PEDIATRIA_SEGUIMENTO));
        install(encounterType("S.TARV: SOLICITAÇÃO ARV", "Solicitação de Medicamentos Anti-Retrovirais", MetadataBundleUtils._EncounterType.S_TARV_SOLICITACAO_ARV));
        install(encounterType("S.TARV: TERMO DE CONSENTIMENTO DE VISITA", "Termo de consentimento de visita domiciliaria/busca activa", MetadataBundleUtils._EncounterType.S_TARV_TERMO_DE_CONSENTIMENTO_DE_VISITA));
        install(encounterType("TRANSFER", "TRANSFER encounter", MetadataBundleUtils._EncounterType.TRANSFER));
        install(encounterType("TUBERCULOSE: LIVRO", "Livro de Registo de Doentes com Tuberculose", MetadataBundleUtils._EncounterType.TUBERCULOSE_LIVRO));
        install(encounterType("TUBERCULOSE: PROCESSO", "TRATAMENTO DE TUBERCULOSE", MetadataBundleUtils._EncounterType.TUBERCULOSE_PROCESSO));
        install(encounterType("TUBERCULOSE: RASTREIO", "TUBURCULOSE: RASTREIO", MetadataBundleUtils._EncounterType.TUBERCULOSE_RASTREIO));
        install(encounterType("TUBERCULOSE: SEGUIMENTO", "Seguimento de doentes com tuberculose", MetadataBundleUtils._EncounterType.TUBERCULOSE_SEGUIMENTO));
