...
INSERT INTO patientflags_flag(`name`,`criteria`,`message`,`enabled`,`evaluator`,`description`,`creator`,`date_created`,`changed_by`,`date_changed`,`retired`,`retired_by`,`date_retired`,`retire_reason`,`uuid`,`priority_id`)
    ...
    (SELECT ob.concept_id FROM obs ob WHERE ob.person_id = p.patient_id AND ob.concept_id=21 AND ob.value_numeric < 8 AND ob.obs_datetime = (SELECT max(obs.obs_datetime) FROM obs obs WHERE obs.person_id=p.patient_id));',
